...
def has_an_infected_neighbor(city, location):
    ...
    if location == 0:
        if city[len(city)-1][0] == "I":
            return True
        elif city[location+1][0] == "I":
            return True
        else:
            return False
...
def advance_person_at_location(city, location, days_contagious):
    ...
    days = city[location][1]
    state = city[location][0]
    if state == "S":
...
def simulate_one_day(starting_city, days_contagious):
    ...
    '''
    city = []
    for i, person in enumerate(starting_city):
        city.append(advance_person_at_location(starting_city, i, days_contagious))
...
def run_simulation(starting_city, days_contagious):
    ...
    days = 0
    while is_transmission_possible(current_city):
        current_city = simulate_one_day(current_city, days_contagious)
        days += 1

    return (current_city, days)
...
def vaccinate_person(vax_tuple):
    ...
    if vax_tuple[0] == "S":
        ...
        if probability < vax_tuple[2]:
            person = ("V", 0)
        else:
            person = ("S", vax_tuple[1])
    else:
        person = (vax_tuple[0], vax_tuple[1])
...
def vaccinate_city(city_vax_tuples, random_seed):
    ...
    for i, person in enumerate(city_vax_tuples):
        city.append(vaccinate_person(city_vax_tuples[i]))
