...
def helper_compute_rectangles(data, bounding_rec):
    """
    Helper function for compute_rectangles, performs the calculations

    Inputs:
        Data (list): A list of tree instances
        bounding_rec (Rectangle): The rectangle within which we
            want to bound ourselves

    Returns: Recursive call to build rectangle list
    """
    if data == []:
        return []

    k_value = 0
    total_sum = sum(point.value for point in data)
    max_row_distortion = 0
    leftover_rec = None
    ...
    for k, leaf in enumerate(data):
        ...
        for row, tree in row_layout:

            row.label = tree.key
            row.color_code = tree.path

            distortion = max(row.width / row.height, row.height / row.width)
            if max_distortion < distortion:
                max_distortion = distortion

            if tree.num_children() == 0:
                current_rec_list.append(row)

            current_child_list.append(tree)

        if k == 0:
            max_row_distortion = max_distortion
        elif max_row_distortion < max_distortion:
            break

        rec_list = current_rec_list
        row_layout_actual = row_layout
        child_list = current_child_list
        k_value = k
        leftover_rec = leftover
        max_row_distortion = max_distortion

    for k, child in enumerate(child_list):
        if child.num_children() != 0:
            bounding_rec = row_layout_actual[k][0]
            new_data = sorted_trees(grandchild for grandchild in child.children)
            rec_list += helper_compute_rectangles(new_data,
                bounding_rec)

    return rec_list + helper_compute_rectangles(data[k_value+1:],
                        leftover_rec)
...
def compute_rectangles(t, bounding_rec_width=1.0, bounding_rec_height=1.0):
    ...
    leaf_list = sorted_trees(child for child in t.children)

    return helper_compute_rectangles(leaf_list, Rectangle((0.0, 0.0),
